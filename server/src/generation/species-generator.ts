import { Species, SpeciesAppearance, SpeciesTrait, SpeciesTraitType } from "@constellation/shared";
import { SeededRandom } from "./random.js";

// Species name prefixes and suffixes for variety
const NAME_PREFIXES = [
  "Astra", "Xeno", "Nebu", "Cosmo", "Stellar", "Quantum", "Chrono",
  "Neo", "Proto", "Arcto", "Hydro", "Pyro", "Cryo", "Electro",
  "Terran", "Jovian", "Solar", "Lunar", "Void", "Astral"
];

const NAME_SUFFIXES = [
  "ans", "ids", "ites", "oids", "ians", "ari", "ini", "oni",
  "ese", "kin", "folk", "born", "spawn", "brood", "kind"
];

// Descriptive adjectives for species
const ADJECTIVES = [
  "Ancient", "Enlightened", "Mysterious", "Noble", "Wise", "Ancient",
  "Peaceful", "Warlike", "Industrious", "Scientific", "Spiritual",
  "Adaptive", "Resilient", "Evolved", "Advanced", "Primitive",
  "Harmonious", "Ambitious", "Curious", "Stoic", "Passionate"
];
...
const TRAIT_DESCRIPTIONS: Record<SpeciesTraitType, string> = {
  [SpeciesTrait.PHOTOSYNTHETIC]: "capable of converting starlight into energy through specialized organs",
  [SpeciesTrait.AQUATIC]: "thriving in oceanic environments with advanced underwater adaptations",
  [SpeciesTrait.SILICON_BASED]: "based on silicon chemistry rather than carbon",
  [SpeciesTrait.EXTREMOPHILE]: "capable of surviving in extreme temperatures and pressures",
  [SpeciesTrait.LONG_LIVED]: "with lifespans measuring in centuries",
  [SpeciesTrait.RAPID_REPRODUCTION]: "with accelerated population growth and short generation cycles",
  [SpeciesTrait.SCIENTIFIC]: "with an innate curiosity and talent for research",
  [SpeciesTrait.INDUSTRIOUS]: "with exceptional skill in manufacturing and construction",
  [SpeciesTrait.EFFICIENT]: "with optimized energy utilization and conservation",
  [SpeciesTrait.ADAPTIVE]: "quickly adapting to new environments and conditions",
  [SpeciesTrait.CURIOUS]: "driven by an insatiable desire to explore and discover",
  [SpeciesTrait.COOPERATIVE]: "naturally inclined toward diplomacy and cooperation",
  [SpeciesTrait.AGGRESSIVE]: "with strong territorial and competitive instincts",
  [SpeciesTrait.PACIFIST]: "philosophically opposed to violence and conflict",
  [SpeciesTrait.XENOPHOBIC]: "distrustful of other species and protective of their own",
  [SpeciesTrait.XENOPHILIC]: "eager to interact with and learn from other species"
};

// Generate skin/surface colors based on body type
function generateColors(random: SeededRandom, bodyType: string): { skinColor: string; eyeColor: string } {
...
function generateTraits(random: SeededRandom, bodyType: string, planetType?: string): SpeciesTraitType[] {
  ...
  const bodyTypeTraits: Record<string, SpeciesTraitType[]> = {
    humanoid: [SpeciesTrait.ADAPTIVE, SpeciesTrait.COOPERATIVE, SpeciesTrait.CURIOUS],
    insectoid: [SpeciesTrait.RAPID_REPRODUCTION, SpeciesTrait.INDUSTRIOUS, SpeciesTrait.COOPERATIVE],
    reptilian: [SpeciesTrait.LONG_LIVED, SpeciesTrait.EXTREMOPHILE, SpeciesTrait.AGGRESSIVE],
    avian: [SpeciesTrait.CURIOUS, SpeciesTrait.ADAPTIVE, SpeciesTrait.EFFICIENT],
    aquatic: [SpeciesTrait.AQUATIC, SpeciesTrait.ADAPTIVE, SpeciesTrait.PACIFIST],
    crystalline: [SpeciesTrait.SILICON_BASED, SpeciesTrait.LONG_LIVED, SpeciesTrait.SCIENTIFIC],
    gaseous: [SpeciesTrait.EFFICIENT, SpeciesTrait.XENOPHILIC, SpeciesTrait.SCIENTIFIC],
    mechanical: [SpeciesTrait.INDUSTRIOUS, SpeciesTrait.EFFICIENT, SpeciesTrait.LONG_LIVED]
  };
  ...
  for (const trait of likelyTraits) {
    if (random.next() < 0.4 && traits.length < numTraits) { // 40% chance for each likely trait
      traits.push(trait);
    }
  }
  ...
  if (planetType && planetTypeTraits[planetType]) {
    for (const trait of planetTypeTraits[planetType]) {
      if (random.next() < 0.6 && !traits.includes(trait) && traits.length < numTraits) {
        traits.push(trait);
      }
    }
  }
...
function generateAppearance(random: SeededRandom, planetType?: string): SpeciesAppearance {
  // Body type influenced by planet type
  let bodyTypes: SpeciesAppearance["bodyType"][] = [
    "humanoid", "insectoid", "reptilian", "avian", "aquatic", "crystalline", "gaseous", "mechanical"
  ];
...
function generateDescription(
  ...
  if (traits.length > 0) {
    ...
    const traitDescs = traits.slice(0, 3).map(trait => TRAIT_DESCRIPTIONS[trait]);
...
export function generateSpecies(
  ...
  const description = generateDescription(name, homeworldName, appearance, traits, random);
...
export function generatePlayerSpecies(
  ...
  const seed = parseInt(playerId.replace(/\D/g, "0"), 10);
  return generateSpecies(speciesId, homeworldName, homeworldId, planetType, seed, playerId);
}
